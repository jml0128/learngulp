...
function changeTextSize(){
	...
	if(document.documentElement.clientWidth*scale<=540){
		for(var i = 0;i<gameBox.length;i++){
			gameBox[i].style.height = window.screen.availHeight / scale+'px';
		}
	}else{
...
!function(e,t){
	function i(){
		var t=n.getBoundingClientRect().width;t/s>540&&(t=540*s);
		var i=t/10;
	...
	var a,
		r=e.document,
		n=r.documentElement,o=r.querySelector('meta[name="viewport"]'),
	...
	if(o){
		...
		if(!s&&!m){
			var v=(e.navigator.appVersion.match(/android/gi),e.navigator.appVersion.match(/iphone/gi)),
				...
				s=v?b>=3&&(!s||s>=3)?3:b>=2&&(!s||s>=2)?2:1:1,
				m=1/b
			}
		...
		e.addEventListener("resize",function(){clearTimeout(a),a=setTimeout(i,300)},!1),
		e.addEventListener("pageshow",function(e){e.persisted&&(clearTimeout(a),a=setTimeout(i,300))},!1),
		//"complete"===r.readyState?r.body.style.fontSize=12*s+"px":r.addEventListener("DOMContentLoaded",function(e){r.body.style.fontSize=12*s+"px"},!1),
		i(),
		d.dpr=e.dpr=s,d.refreshRem=i,d.rem2px=function(e){
